...
#include "PalaceLattice/GridPathFinding.h"
#include "PalaceLattice/TilePawn.h"
#include "PalaceLattice/BasedTile.h"
	...
	for (TActorIterator<ABasedTile> BasedTileIterator(GetWorld()); BasedTileIterator; ++BasedTileIterator)
	{
		...
		if (GridEdgeArray.IsValidIndex(Index))
		{
			...
			if (bAutoCalculateCostsBaseOnHeight)
			{
				...
				if (ActorHeight != 0.f)
				{
					int32 TileIndex = BasedTileIterator->TileIndex;
					if (FieldLocationArray.IsValidIndex(TileIndex)) { FieldLocationArray[TileIndex].Z = ActorHeight; }
				}
			}
		}
	}
